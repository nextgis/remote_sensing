...
The results of first version of this approach are presented below. Some technical details of the system are in Appendix.
 ...
 * the approach can handle simple objects only (it actually can handle complex objects as well, but then the system isn't simple and the advantages disappear);
 * new types of data (new imagery set, new objects etc.) require new exploration and new reconstruction of the system.

## First results

We have created first version of the system. The system has several parameters that have to be adjusted. In the current version parameters' settings are preliminary. We think that well-adjusted parameters will further improve the quality, but not dramatically, so we can use current parameters for proof of the concept.

We used two scenes (http://kolesov.nextgis.com/resource/100/display?base=osm-mapnik&lon=-123.8228&lat=40.1049&angle=0&zoom=11&styles=569,579,555,549) as testing area to setup the system parameters (see details in Appendix). 

We then applied the system to other scenes taken in 11 jul 2016. The resulst can be viewed here http://kolesov.nextgis.com/resource/100/display?base=osm-mapnik&lon=-123.8385&lat=40.6925&angle=0&zoom=8&styles=595,551,555,527,549,529,545,553,563,521,535,559,523,561,539,543,547,531,537,533,541,557,525
...
The layers contains polygons that are classified as 'greenhouses or building' using defined rules. The number in the layer name indicates 'certainty'. Layer 'nextgis-answer:0.75' contains few polygons, but the system is more certain that the polygons are buildings/greenhouses. Layer
'nextgis-answer:0.25' contains lots of polygons, but the system isn't 'certain' about the answer. 'Certainties' aren't perfectly fitted, but they are good enough as proof of the concept).

## Appendix. Structure of the system.

### Architecture

The classification system is constructed as rule-based system (actually only one rule is currently used, but the system can handle multiple rules).
...
The rule uses concepts '`BRIGHT`', '`SPOT`', '`SMALL`', '`SIMPLE FORM`'. We have to give them strong definitions before using them in our system.

Here is the definition process.

*BRIGHT*

`BRIGHT` is the simplest concept because brightness is encoded in pixel values: every pixel of PlanetLabs image has three components (RGB). Typical example of 'bright' pixel is something like (850-850-600). But of course if we take pixel (849-849-599), we can't say that this pixel is not bright. Obviously strict boundary between bright and dark pixels doesn't exist: there are a lot of values that are nether bright nether dark (or they are dark and bright in the same time). So we can use fuzzy logic approach to handle such uncertainties.

Therefore we need to construct membership function for fuzzy set "`BRIGHT PIXEL`". This function indicates degree of brightness, it is equal to 1.0 for bright pixels and 0.0 for dark pixels.

To construct this function we took some training areas (bright and dark) and constructed auxiliary fuzzy sets "`BRIGHT RED COLOR`", "`BRIGHT GREEN COLOR`" and "`BRIGHT BLUE COLOR`". We decided to use logistic function to fit the membership function. We found parameters of the logistic function where bright areas have values of the membership function near 1.0.
...
Similar membership functions were created for other fuzzy sets.
...
Concept `SPOT` is more complex to define then concept `BRIGHT`. We can describe `SPOT` as "group of connected pixels of nearly same color/brightness". We decided to find such groups using Region Growing algorithm. This algorithm is a kind of a clustering algorithm that uses spectral and spatial information during clusterization process. The algorithm isn't fuzzy, so the clusters have strict boundaries, but we can create fuzzy set "`BRIGHT SPOT`" that folds both concepts `BRIGHT` and `SPOT`.

So as a result of this step scenes are divided on fuzzy sets "`BRIGHT SPOT`", they looks like continuous areas with hard boundaries but every area contains a value in [0; 1]. This value is the degree of the spot's brightness (max value of membership function "`BRIGHT PIXEL`" in the area).

Example of resulting spots are presented below:
...
The next concept is "`SMALL`" spot. This concept is simple: we can calculate areas of spots and compare them with usual areas of greenhouses. We used the next membership function for fuzzy set "`SMALL SPOT`":
```
...
We have defined all concepts used in the rule. As result we can concatenate all parts of the rule using functions of fuzzy logic. In out case we have only one logic finction (`AND`). We used `min` function as logic `AND`.

### Ways to improvements

There are some steps that can be done to improve the quality of classification:

 * We didn't fit the membership functions very well. Good fitting on large training sets could impove the result.
 * We haven't studied different clusterization methods for create '`SPOTS`'. Probably different algorithms or their parameters can produce more adequate '`SPOTS`'.
 * We haven't studied different realisations of `AND` function. 

We think that these studies are needed and that the result could be better. But the final result strongly depends on the used data (PlanetLabs imagery), therefore the main improvement can be achieved by more adequate input data.
